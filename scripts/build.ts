...
async function gitPrepare() {
    ...
    await exec("echo", [`"TAG_NAME=${tagName}"`, ">>", "$GITHUB_ENV"]);
    return tagName;
}
...
async function build() {
    ...
    try {
        ...
        fs.writeFileSync("dist/manifest.json", JSON.stringify(mds, null, 2));
        fs.writeFileSync("dist/manifest.min.json", JSON.stringify(mds));
    } catch (error) {
